...
Keep rules in a versioned table rather than embedded in the code, validate that only active, approved rules get applied, record the assumptions behind each run, and package the whole thing for reviewer sign-off along with a movement analysis showing what changed versus the prior version.
